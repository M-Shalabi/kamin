...
export default async function Page({ searchParams }: { searchParams: Promise<{ kind?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm" style={{ color: "var(--muted)" }}>{rows.length} pooled orders worth <Money v={total} /> a year. A manufacturing gap has no supported manufacturer or assembler; a supply gap has no supported supplier of any class. Spec says whether the best supplier is verified at the stated specification or only declares the subheading.</p>
