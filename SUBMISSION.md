...
**Principal risks and mitigations:** several Saudi government portals apply bot protection or render client-side, mitigated by using captured JSON endpoints and paid scraping fallbacks priced in advance rather than fighting protections live. Arabic–English product normalisation is the critical technical path and is prototyped first. Published figures conflict across sources (national catalogue counts, mandatory-list sizes), so every number presented on stage is reconciled to a single cited source beforehand.
