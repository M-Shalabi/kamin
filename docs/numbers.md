...
Generated 2026-09-02 13:59 UTC by `bun run numbers` from the live database. Regenerate after any run; never edit the values by hand.

| Number | Value | Produced by |
|---|---|---|
| Suppliers on the map | 15,032 | `select count(*) as v from suppliers where id not like 'test:%'` |
...
| … absent from Tarmeez (found through MLCP, Made in Saudi or the hunt) | 159 | `select count(*) as v from suppliers where not in_tarmeez and id not like 'test:%'` |
| … discovered by the long-tail hunt, in no registry | 17 | `select count(*) as v from suppliers where source = 'hunt'` |
| Suppliers with a capability in the valve, pump and fitting slice | 424 | `select count(distinct supplier_id) as v from capabilities where left(hs6, 4) in ('8481', '8413', '7307') and supplier_id not like 'test:%'` |
| Capabilities on the map | 51,960 | `select count(*) as v from capabilities where supplier_id not like 'test:%'` |
| … in the slice | 744 | `select count(*) as v from capabilities where left(hs6, 4) in ('8481', '8413', '7307') and supplier_id not like 'test:%'` |
...
| Evidence records, Tier 2 | 52,026 | `select count(*) as v from evidence e join capabilities c on c.id = e.capability_id where e.tier = 2 and c.supplier_id not like 'test:%'` |
| Evidence records, Tier 3 | 341 | `select count(*) as v from evidence e join capabilities c on c.id = e.capability_id where e.tier = 3 and c.supplier_id not like 'test:%'` |
...
| Suppliers investigated by a Detective | 86 | `select count(*) as v from suppliers where detective_status = 'ok' and id not like 'test:%'` |
| … Detective runs that errored | 4 | `select count(*) as v from suppliers where detective_status = 'error'` |
| Slice capabilities audited | 323 | `select count(*) as v from capabilities where audit_run_id is not null and left(hs6, 4) in ('8481', '8413', '7307')` |
| … supported | 210 | `select count(*) as v from capabilities where verdict = 'supported' and left(hs6, 4) in ('8481', '8413', '7307')` |
| … refuted | 14 | `select count(*) as v from capabilities where verdict = 'refuted' and left(hs6, 4) in ('8481', '8413', '7307')` |
...
| Supported capabilities backed by Tier 1 or 2 (count toward coverage) | 186 | `select count(*) as v from capabilities c where c.verdict = 'supported' and left(hs6, 4) in ('8481', '8413', '7307') and exists (select 1 from evidence e where e.capability_id = c.id and e.tier <= 2)` |
| Supported by class: manufacturer | 184 | `select count(*) as v from capabilities where verdict = 'supported' and class = 'manufacturer' and left(hs6, 4) in ('8481', '8413', '7307')` |
...
| Supported by class: authorised distributor | 10 | `select count(*) as v from capabilities where verdict = 'supported' and class = 'authorised_distributor' and left(hs6, 4) in ('8481', '8413', '7307')` |
| Supported by class: trader | 16 | `select count(*) as v from capabilities where verdict = 'supported' and class = 'trader' and left(hs6, 4) in ('8481', '8413', '7307')` |
...
| Orders with a supplier only at category level (the Detectives' next queue) | 70 | `select count(*) as v from pooled_orders where spec_status = 'category' and title not like 'test %'` |
...
| Discovery lift: suppliers absent from Tarmeez with a supported capability in the slice | 13 | `select count(distinct c.supplier_id) as v from capabilities c join suppliers s on s.id = c.supplier_id where not s.in_tarmeez and s.id not like 'test:%' and c.verdict = 'supported' and left(hs6, 4) in ('8481', '8413', '7307')` |
