...
Generated 2026-09-03 19:59 UTC by `bun run numbers` from the live database. Regenerate after any run; never edit the values by hand.
...
| Capabilities on the map | 51,943 | `select count(*) as v from capabilities where supplier_id not like 'test:%'` |
| … in the slice | 739 | `select count(*) as v from capabilities where left(hs6, 4) in ('8481', '8413', '7307') and supplier_id not like 'test:%'` |
...
| Evidence records, Tier 3 | 419 | `select count(*) as v from evidence e join capabilities c on c.id = e.capability_id where e.tier = 3 and c.supplier_id not like 'test:%'` |
...
| Slice capabilities audited | 318 | `select count(*) as v from capabilities where audit_run_id is not null and left(hs6, 4) in ('8481', '8413', '7307')` |
| … supported | 199 | `select count(*) as v from capabilities where verdict = 'supported' and left(hs6, 4) in ('8481', '8413', '7307')` |
| … refuted | 15 | `select count(*) as v from capabilities where verdict = 'refuted' and left(hs6, 4) in ('8481', '8413', '7307')` |
| … still pending after audit | 109 | `select count(*) as v from capabilities where audit_run_id is not null and verdict = 'pending' and left(hs6, 4) in ('8481', '8413', '7307')` |
| Supported capabilities backed by Tier 1 or 2 (count toward coverage) | 179 | `select count(*) as v from capabilities c where c.verdict = 'supported' and left(hs6, 4) in ('8481', '8413', '7307') and exists (select 1 from evidence e where e.capability_id = c.id and e.tier <= 2)` |
| Supported by class: manufacturer | 176 | `select count(*) as v from capabilities where verdict = 'supported' and class = 'manufacturer' and left(hs6, 4) in ('8481', '8413', '7307')` |
...
| Supported by class: authorised distributor | 5 | `select count(*) as v from capabilities where verdict = 'supported' and class = 'authorised_distributor' and left(hs6, 4) in ('8481', '8413', '7307')` |
| Supported by class: trader | 18 | `select count(*) as v from capabilities where verdict = 'supported' and class = 'trader' and left(hs6, 4) in ('8481', '8413', '7307')` |
...
| Coverage at the stated specification, spend-weighted share of pooled annual demand (the headline figure) | 2.1% | `packages/core/src/match/coverage.ts (bun run coverage)` |
| … orders covered at the stated specification | 6.9% | `same` |
...
| Capabilities carrying attributes written by the Specifier from catalogues | 36 | `select count(distinct c.id) as v from capabilities c join evidence e on e.capability_id = c.id join runs r on r.id = e.run_id where r.role = 'specifier'` |
...
| Typed relations in the graph (brands distributed, groups, certifiers, standards, materials, processes) | 70 | `select count(*) as v from relations r join suppliers s on s.id = r.subject_id where s.id not like 'test:%'` |
| Investment cases written by the Advisor | 2 | `select count(*) as v from gap_cases` |
| Matches written | 1,440 | `select count(*) as v from matches m join pooled_orders o on o.id = m.pooled_order_id where o.title not like 'test %'` |
...
| auditor: seconds per run on ollama:qwen3.5:9b (417 runs) | 22 s, 1,121 tokens in, 266 out | `runs and run_steps (bun run cost)` |
| coordinator: seconds per run on ollama:qwen3.5:9b (125 runs) | 71 s, 4,568 tokens in, 356 out | `runs and run_steps (bun run cost)` |
| detective: seconds per run on ollama:qwen3.5:9b (97 runs) | 98 s, 5,697 tokens in, 934 out | `runs and run_steps (bun run cost)` |
| specifier: seconds per run on ollama:qwen3.5:9b (161 runs) | 57 s, 2,899 tokens in, 507 out | `runs and run_steps (bun run cost)` |
