...
console.log(`coverage at spec: ${(c.coverage_spec * 100).toFixed(1)}% of USD ${(c.spend_total / 1e6).toFixed(1)}M (${(c.line_coverage_spec * 100).toFixed(1)}% of ${c.line_total} pooled orders) | type verified: ${(c.coverage_type * 100).toFixed(1)}% of spend | category level: ${(c.coverage * 100).toFixed(1)}% of spend, ${(c.line_coverage * 100).toFixed(1)}% of orders | manufacturing gaps: ${c.manufacturing_gaps} | supply gaps: ${c.supply_gaps}`);
