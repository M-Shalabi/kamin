 ...
 * Usage: bun run spec:pass [limit] [--hs6 848180] [--deep] [--redo]
 *   --deep follows catalogue and PDF links one level below the product pages; --redo re-reads suppliers already read.
 */
...
const deep = args.includes("--deep"), redo = args.includes("--redo");
const targets = await sql<{ id: string; name: string | null; website: string | null; usd: number; hs6s: string[] }[]>`
  ...
  where c.verdict = 'supported' and s.id not like 'test:%'
    and (${redo} or not exists (select 1 from runs r where r.role = 'specifier' and r.input_ref = s.id and r.status = 'ok'))
  group by s.id, s.name_en, s.name_ar, s.website
  order by (s.website is not null) desc, usd desc limit ${limit}`;
console.log(`specifier${deep ? " (deep)" : ""}: ${targets.length} suppliers, ${targets.filter((t) => t.website).length} with a website`);
const started = Date.now(); let attributed = 0, evidence = 0, failed = 0;
for (const [i, t] of targets.entries()) {
  try {
    const r = await runSpecifier(sql, t.id, { hs6s: t.hs6s, deep });
