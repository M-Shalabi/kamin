...
export async function invokeStructured<T>(model: BaseChatModel, messages: BaseMessage[], opts: StructuredOptions<T>): Promise<T> {
  ...
  for (let attempt = 0; attempt <= (opts.maxRetries ?? 1); attempt++) {
    const msg = (await tooled.invoke(history, opts.config)) as AIMessage;
    ...
    history.push(msg, new HumanMessage(`Your previous reply did not match the required schema: ${lastIssues}. Call the ${opts.name} tool again with every field present, using null for unknown values. /no_think`));
  }
