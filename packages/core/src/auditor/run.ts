...
export function auditorPrompt(cap: CapabilityForAudit): { system: string; human: string } {
  ...
  return {
    system: [
      ...
      'The reply has exactly this shape, all five keys present: {"analysis": "...", "real": {"verdict": "supported|refuted|unknown", "reasoning": "...", "killer_evidence": null}, "at_spec": {"verdict": "supported|refuted|unknown", "reasoning": "..."}, "local": {"class": "manufacturer|assembler|authorised_distributor|trader", "reasoning": "..."}, "confidence": 0.0}',
      "/no_think",
    ].join("\n"),
...
export async function runAuditor(db: Sql, capabilityId: string, opts: { sink?: (line: string) => void } = {}): Promise<{ runId: string; verdict: "supported" | "refuted" | "pending"; class: string; confidence: number }> {
  ...
  return withRun(db, { role: "auditor", inputRef: capabilityId, model: modelRefFor("auditor") }, async (runId, handler) => {
    ...
    const lenses = await invokeStructured(getChatModel("auditor"), [new SystemMessage(p.system), new HumanMessage(p.human)], {
      name: "audit_verdict", description: "The three-lens audit of one capability claim", toolSchema: AuditVerdict, parseSchema: AuditVerdictLoose, preferJsonText: true, maxRetries: 2,
