...
export async function coverage(db: Sql, opts: { onlyOrderIds?: string[] } = {}) {
  ...
  const [r] = await db<{ spend_total: number; spend_covered: number; line_total: number; line_covered: number; manufacturing_gaps: number; supply_gaps: number; spend_at_spec: number; line_at_spec: number }[]>`
