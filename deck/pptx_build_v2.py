# -*- coding: utf-8 -*-
"""Builds an editable PowerPoint from the deck. Version 2.

What v1 got wrong: it named Thmanyah and Archivo, and neither font exists on
this machine or on most machines that would open the file. The XML was correct
(rtl and complex-script typefaces were both set) but every glyph fell back to a
substitute, so the deck arrived looking nothing like the design.

v2 fixes three things:
  1. Fonts are IBM Plex Sans Arabic and IBM Plex Mono, which the product itself
     uses, are OFL licensed, and are now installed locally.
  2. The complex-script run properties are written in schema order. OOXML
     requires latin, then ea, then cs; v1 wrote cs before ea, which a strict
     reader is entitled to reject.
  3. The fonts are embedded in the package, so the deck renders correctly on a
     machine that has never heard of IBM Plex.

Text stays native and editable. The vector pieces (the map, the funnel, the
product screens) come in as pictures, because they were never text.
Run: python3 pptx_build_v2.py
"""
import copy, os
...
W, H = 1600, 900
# Resolve art relative to this file, so the build works from any cwd.
HERE = os.path.dirname(os.path.abspath(__file__))
ART = os.path.join(HERE, 'pptx-art')
...
KUFI, BODY, MONO = 'IBM Plex Sans Arabic', 'IBM Plex Sans Arabic', 'IBM Plex Mono'
...
def txt(s, x, y, w, text, size, color=T, bold=False, font=BODY, align='r',
    ...
    for tag in ('ea', 'cs'):          # schema order: latin, ea, cs
...
def pic(s, name, x, y, w, h=None):
    # names may be bare (face-ali.jpg) or already under ART
    for cand in (name, os.path.join(ART, name), os.path.join(HERE, name)):
